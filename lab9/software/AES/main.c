...
#define to_sw_port (volatile char*) 0x00000030 // actual address here
#define to_sw_sig (volatile char*) 0x00000020 // actual address here
...
void MixColumns(uchar* msg)
{
	...
	for ( i=0; i<4; i++){
		for( j=0; j<4; j++){
			idx = 4*i +j;
			printf("j:%d idx:%d temp: %x \n",j, idx, msg[idx]);
			if(j == 0) temp[idx] = (xtime(msg[idx])) ^ ((xtime(msg[idx+1])) ^ msg[idx+1]) ^ msg[idx+2] ^ msg[idx+3];
			if(j == 1) temp[idx] = msg[idx-1] ^ (xtime(msg[idx])) ^ ((xtime(msg[idx+1]))^msg[idx+1]) ^ msg[idx+2];
			if(j == 2) temp[idx] = msg[idx-2] ^ msg[idx-1] ^ (xtime(msg[idx])) ^ ((xtime(msg[idx+1]))^msg[idx+1]);
			if(j == 3) temp[idx] = ((xtime(msg[idx-3]))^msg[idx-3]) ^ msg[idx-2] ^ msg[idx-1] ^ (xtime(msg[idx]));
			printf("temp after: %x \n", temp[idx]);
		}
	}
